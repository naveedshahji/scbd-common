...
const  binding  = computed({
  get()      {  return model.value as string; 
  },
...
const onEditorReady=(ed: any) =>{
    ...
    upload() {
      ...
      return this.loader.file.then((file: File) => {
        ...
        return api.attachments.uploadTempFile(props.identifier, file, file.name)
          ...
          .catch((error : any) => {
            uploadErrors.value.push({file: file.name });
  ...
  ed.editing.view.document.on('paste', (eventInfo: any, data: any) => {
     // console.debug('paste', eventInfo, data)
  });

  ed.editing.view.document.on('drop', async (eventInfo: any, data: any) => {
    if(data.dataTransfer){
      isUploadingFile.value = true;

      const fileUploads = Array.from(data.dataTransfer.files).map(async (file: File) => {
        ...
        return api.attachments.uploadTempFile(props.identifier, file, file.name)
          .then((success: any) =>{
            const viewFragment = ed.data.processor.toView(
              `<span class="me-2">&nbsp;<a rel="noopener noreferrer" target="_blank" href="${success.url}">${success.filename}</a>&nbsp;</span>`
            );
  ...
  ed.model.document.on('change:data', (eventInfo: any, data: any) => {
    // console.debug('change', eventInfo, data)
  });

  function onEditorImageUploaded(eventInfo: any, name: string, value: any, oldValue: any) {
  ...
  const onEditorFocus=(event: Event, editor: any) => {
    // console.debug( 'Editor focused.', { event, editor } );
    emit('onEditorFocus', event, editor);
  };

  const onEditorBlur=(event: Event, editor: any) => {
    emit('onEditorBlur', event, editor);
  };

  const onEditorInput=(data: any, event: Event, editor: any) => {
    emit('onEditorInput', event, editor);
  };

const onEditorDestroy = (editor: any) => {
  emit('onEditorDestroy', editor);
};

  const onFileUpload = (params: any) => {   emit('onFileUpload', params);};
